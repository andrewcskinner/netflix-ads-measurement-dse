...
Each runner returns a `Result`: a headline, a raw-number dict for the LLM to
summarize (and for a deterministic fallback), an optional table + column config,
an optional Plotly figure, and the provenance (which SQL and which functions
ran, with the parameters that were bound).
"""
...
class Result:
    headline: str
    takeaway: str                       # one deterministic decision line
...
def run_incrementality(enums, advertiser_name=None, **_) -> Result:
    ...
    takeaway = ("This campaign is driving incremental conversions — keep or scale it."
                if row.significant and row.lift > 0 else
                "Lift is not statistically distinguishable from zero — do not scale on this read.")
    stats_d = {
        "scope": scope,
    ...
    return Result(headline, takeaway, stats_d,
...
def run_inventory_demand(enums, metric="sell_through", **_) -> Result:
    ...
    headline = (f"Platform sell-through {plat_fill:.0%}. Hottest: {hot.cohort_name} "
                ...
                f"{cold.cohort_name} ({cold.fill_rate:.0%}).")
    takeaway = (f"Spin-up room is largest in {unsold.cohort_name} "
                f"({unsold.unfilled_slots:,.0f} unsold slots); throttle or repackage the "
                f"softest cohorts before they drag clearing prices down.")
    stats_d = {
    ...
    return Result(headline, takeaway, stats_d, {"metric": metric},
...
def run_frequency_elasticity(enums, category=None, **_) -> Result:
    ...
    if not fit.get("ok"):
        ...
        return Result(f"Not enough matched frequency data in {category} for a stable fit.",
                      "Gather more exposures in this category before setting a frequency cap.",
    ...
    takeaway = (f"Cap frequency near {practical_cap:.0f} exposures; beyond that, spend is "
                f"better moved to reach or a fresh cohort.")
    stats_d = {
        "category": category,
    ...
    return Result(headline, takeaway, stats_d, {"category": category},
...
def run_ab_test(enums, advertiser_name=None, **_) -> Result:
    ...
    takeaway = (f"Ship arm {winner} (“{wname}”) to 100% of rotation." if t.significant
                else "No significant winner — keep the 50/50 split or run longer.")
    stats_d = {
    ...
    return Result(headline, takeaway, stats_d, {"advertiser_name": advertiser_name},
...
def run_cohort_recommendation(enums, advertiser_name=None, **_) -> Result:
    ...
    if scored.empty:
        return Result(f"Not enough measured data in the {arow.category} category.",
                      "Collect more holdout-measured conversions before recommending cohorts.",
    ...
    takeaway = (f"Buy {best.cohort_name} first — projected "
...
ANALYSES: dict[str, Analysis] = {
    a.key: a for a in [
        ...
        Analysis("inventory_demand", "Inventory demand & sell-through",
                 ...
                 "Optional param: metric (sell_through | clearing_price).",
                 ["metric"],
...
def build_tool_schema(enums: dict) -> dict:
    """OpenAI function schema constraining analysis_type and every parameter to
    enumerated values from the registry and the warehouse."""
    analysis_lines = "; ".join(f"{k} = {a.description}" for k, a in ANALYSES.items())
    return {
        "type": "function",
        "function": {
            "name": TOOL_NAME,
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "category": {"type": "string", "enum": enums["category"],
                                 "description": "Advertiser category (frequency_elasticity)."},
                    "cohort_name": {"type": "string", "enum": enums["cohort_name"],
                                    "description": "Session cohort, when the question names one."},
                    "metric": {"type": "string", "enum": enums["metric"],
                               "description": "Inventory metric to lead with (inventory_demand)."},
                    "intent": {"type": "string",
...
_KEYWORDS = [
    ("cohort_recommendation", ("recommend", "should buy", "should spend", "where to buy",
                               "where should", "allocate", "budget", "next $")),
    ("frequency_elasticity", ("frequency", "how many times", "how often", "elasticit",
                              "saturat", "optimal freq", "frequency cap")),
    ("ab_test", ("a/b", "ab test", "creative", "which ad won", "which creative", "arm ",
                 "winner", "variant")),
    ("inventory_demand", ("inventory", "demand", "sell-through", "sell through", "fill rate",
                          "undersell", "undersold", "unsold", "spin up", "spin-up",
                          "spin down", "ecpm", "clearing price", "daypart")),
    ("incrementality", ("lift", "incremental", "working", "effective", "causal", "holdout",
                        "convert", "roi", "impact")),
]
